...
import io.github.artsobol.progressservice.feature.training.sessionexercise.dto.request.UpdateTrainingSessionExerciseCommentRequest;
...
public class TrainingSessionExerciseController {
    ...
    @PatchMapping("/training-session-exercises/{sessionExerciseId}/comment")
    public TrainingSessionExerciseResponse updateComment(
            @PathVariable @Positive Long sessionExerciseId,
            @AuthenticationPrincipal UserPrincipal principal,
            @RequestBody UpdateTrainingSessionExerciseCommentRequest request
    ) {
        return trainingSessionExerciseService.updateComment(sessionExerciseId, principal.userId(), request.comment());
    }
}
